...
(function ($) {
	$.fn.amazingaudioplayer = function (options) {
		...
		var PlayerSkin = function (amazingPlayer, container, options, id) {
			...
			var formatSeconds = function (secs) {
				...
				if (instance.options.customtimestyle)
					return instance.options.timestyle.replace("%HOURS%", hours).replace("%MINUTES%",
			...
			if (this.options.showimage) {
				...
				this.container.on("amazingaudioplayer.updateinfo", function (event, data) {
					if (data.image.length > 0)
						instance.$image.html("<div class='amazingaudioplayer-image-space' style='display:inline-block;vertical-align:middle;height:100%;'></div><img alt='" + amEscapeHtml(data.title) + "' src='" + data.image + "' style='width:100%;max-width:100%;vertical-align:middle;' />");
					else
			...
			if (!isTouch || !this.options.disablehoverontouch)
				$play.hover(function () {
					$(this).css({
			...
			$play.click(function () {
				if (instance.options.reloadstream)
					instance.amazingPlayer.reloadPlayAudio();
				else
			...
			this.container.on("amazingaudioplayer.played", function (event, currentItem) {
				...
				if (instance.options.enablega && instance.options.gatrackingid && window.ga)
					ga("send", "event", instance.options.gacategory, "play", instance.amazingPlayer.elemArray[currentItem].title, instance.options.gaeventvalue)
			});
			this.container.on("amazingaudioplayer.paused",
				...
				if (instance.options.enablega && instance.options.gatrackingid && window.ga)
					ga("send", "event", instance.options.gacategory, "pause", instance.amazingPlayer.elemArray[currentItem].title, instance.options.gaeventvalue)
			});
			...
			this.container.on("amazingaudioplayer.ended", function (event, currentItem) {
				if (instance.options.enablega && instance.options.gatrackingid && window.ga)
					ga("send", "event", instance.options.gacategory, "end", instance.amazingPlayer.elemArray[currentItem].title, instance.options.gaeventvalue)
			});
			...
			if (this.options.showvolume && !AmazingAudioPlatforms.isIOS() && !AmazingAudioPlatforms.isAndroid()) {
				...
				if (this.options.showvolumebar) {
					...
					if (!isTouch || !instance.options.disablehoverontouch)
				...
				this.container.on("amazingaudioplayer.setvolume", function (event, volume) {
					...
					if (instance.options.showvolumebar)
						if (instance.options.volumebarposition ==
							"top" || instance.options.volumebarposition == "bottom")
							instance.$volumeBarAdjustActive.css({
								height: Math.round(volume * 100) + "%"
							});
						else
			...
			if (this.options.showprogress) {
				...
				if (instance.options.progressinbar)
					$progress.appendTo($bar);
				else
					$progress.appendTo(this.playerwrapper);
				$progress.css({
					display: "block",
					cursor: "pointer",
					overflow: "hidden",
			...
			if (this.options.showtracklist) {
						...
						if (instance.options.tracklistitem < instance.amazingPlayer.elemLength)
							if (instance.options.tracklistscroll) {
								$.amascrollbar_clear(instance.$tracklist,
									instance.$tracks);
								if (instance.options.tracklistitem < visibleCount)
									$.amascrollbar(instance.$tracklist, instance.$tracks, {
				...
				for (var i = 0; i < this.amazingPlayer.elemLength; i++) {
					...
					if (!title && this.options.changeemptytitletospace)
						title = "&nbsp;";
					...
					if (!isTouch || !instance.options.disablehoverontouch)
						$track.hover(function () {
							...
							if ($(this).data("order") != instance.amazingPlayer.currentItem)
								$(this).addClass("amazingaudioplayer-track-item-hoverover")
						}, function () {
							if ($(this).data("order") !=
				...
				this.drawNavArrows = function () {
					...
					if (!isTouch || !this.options.disablehoverontouch)
						this.$tracklistarrowprev.hover(function () {
							$(this).css({
								"background-position": "left bottom"
							})
						}, function () {
					...
					if (!isTouch || !this.options.disablehoverontouch)
						this.$tracklistarrownext.hover(function () {
							$(this).css({
								"background-position": "right bottom"
							})
						}, function () {
		...
		FlashHTML5Player.prototype = {
			...
			load: function (audioItem) {
				...
				if (this.amazingPlayer.options.forcefirefoxflash && AmazingAudioPlatforms.isFirefox() &&
					...
					this.isHtml5 = false;
				if (this.amazingPlayer.options.forcechromeflash && AmazingAudioPlatforms.isChrome() && AmazingAudioPlatforms.flashInstalled() && !AmazingAudioPlatforms.isMobile())
					this.isHtml5 = false;
				if (this.amazingPlayer.options.forceflash && AmazingAudioPlatforms.flashInstalled() && !AmazingAudioPlatforms.isMobile())
					this.isHtml5 = false;
				if (this.amazingPlayer.options.forcehtml5)
					this.isHtml5 = true;
				if (this.isHtml5) {
					if (!this.html5Object)
					...
					if (this.audioItem.title)
						this.html5Object.attr("title", this.audioItem.title);
					for (i = 0; i < audioSource.length; i++)
						if (audioSource[i].type == "audio/mpeg" && this.supportMp3 || audioSource[i].type == "audio/ogg" && this.supportOgg)
							this.html5Object.append("<source src='" + audioSource[i].src + "' type='" + audioSource[i].type + "'></source>");
					this.amazingPlayer.playProgress(0, 0);
					this.html5AudioLoaded = false;
			...
			flashLoadAudio: function (mp3Src, playAudio) {
				...
				if (!AmazingFlashAudioPlayerReady[this.id]) {
					...
					if (!this.flashObject)
						this.flashObject = window.AmazingSWFObject.getObjectById("amazingflashaudioplayer-" + this.id);
			...
			play: function (startTime, duration) {
				...
				if (this.isHtml5) {
					...
					if (promise !==
						undefined && "catch" in promise)
						promise["catch"](function () {
							instance.amazingPlayer.pauseAudio()
						});
					if (startTime > 0)
						this.html5Object.get(0).currentTime = startTime;
					...
					if (startTime > 0 && startTime < duration)
						this.flashObject.setTime(startTime /
							duration)
			...
			setSeconds: function (seconds) {
				if (this.isHtml5)
					this.html5Object.get(0).currentTime = seconds
			},
			getVolume: function () {
				if (this.isHtml5)
			...
			setVolume: function (val) {
				...
				else if (this.flashObject)
					this.flashObject.setVolume(val)
			},
			setPlaybackRate: function (val) {
				if (this.isHtml5)
					this.html5Object.get(0).playbackRate =
						val
			}
		};
		...
		AmazingAudioPlayer.prototype = {
			...
			initData: function (onSuccess) {
				this.readTags();
				if (this.options.remote && this.options.remote.length > 0)
					this.readRemote(onSuccess);
				else
					onSuccess(this)
			},
			readTags: function () {
			...
			processInit: function () {
				...
				if (instance.options.saveposincookie)
					instance.container.on("amazingaudioplayer.playprogress",
						function (event, data) {
						var current = isNaN(data.current) ? 0 : data.current;
				...
				if ("firstaudioid" in params && !isNaN(params["firstaudioid"]) && params["firstaudioid"] >= 0 && params["firstaudioid"] < instance.elemLength)
					firstId = parseInt(params["firstaudioid"]);
				...
				if ("start" in params && !isNaN(params["start"]) && params["start"] >= 0)
					startTime = parseInt(params["start"]);
				if (instance.options.autoplay && !AmazingAudioPlatforms.autoplayDisabled())
					window.setTimeout(function () {
						instance.playAudio(startTime)
					},
						instance.options.autoplaytimeout);
				else if (instance.currentItem >= 0 && startTime > 0 && startTime < instance.elemArray[instance.currentItem].duration)
					instance.setTime(startTime / instance.elemArray[instance.currentItem].duration);
			...
			audioRun: function (index, autoPlay) {
				...
				if (index == -2)
					...
					nextItem = this.currentItem >= this.elemLength -
				...
				if (this.elemArray[this.currentItem].live) {
					...
					if (src.indexOf("radionomy.com") > 0 && this.options.updateradionomy) {
						if (this.elemArray[this.currentItem].radionomyradiouid)
							this.updateLiveRadionomy()
					} else if (src.indexOf("shoutcast.com") > 0 && this.options.updateshoutcaststation) {
						if (this.elemArray[this.currentItem].radionomyradiouid)
							this.updateLiveShoutcastStation()
						...
						if (this.options.shoutcastjsonenabled)
							...
							this.updateLiveShoutcastByHTML()
				}
				this.checkMK()
			},
			...
			drawShoutcast: function (data) {
				...
				if (instance.options.showliveplayedlist) {
					...
					if (instance.options.addwm && count > 2)
						count = 2;
					...
					for (var i = 0; i < count; i++) {
						...
						} else if (data[i].playedatstring)
							text += '<div class="amazingaudioplayer-playedlist-item-shoutcast-time">' + data[i].playedatstring + "</div>";
			...
			updateLiveShoutcastByHTML: function () {
				if (typeof wonderplugin_audio_ajaxobject == "undefined" && !this.options.shoutcastservice)
					return;
				var instance = this;
				clearTimeout(instance.liveUpdateTimeout);
				instance.liveUpdateTimeout = null;
				...
				if (a.prop("port"))
					src += ":" + a.prop("port");
				src += "/played";
				if (instance.options.shoutcastcustomplayedurl && instance.options.shoutcastcustomplayedurl.length > 0)
					src = instance.options.shoutcastcustomplayedurl;
				var args = {
					type: "POST",
					data: {
				...
				if (typeof wonderplugin_audio_ajaxobject != "undefined") {
					...
					args.url = instance.options.shoutcastservice;
				$.ajax(args).done(function (data) {
					if (data && $.isArray(data) && data.length > 0)
						instance.drawShoutcast(data);
					instance.liveUpdateTimeout = setTimeout(function () {
			...
			updateLiveRadionomy: function () {
					...
					if (tracks && tracks.length > 0) {
						...
						if (track && track.length > 0) {
							...
							if (cover && cover.length > 0)
								instance.elemArray[instance.currentItem].image = cover;
							instance.container.trigger("amazingaudioplayer.updateinfo", instance.elemArray[instance.currentItem]);
				...
				if (instance.options.showliveplayedlist && (typeof wonderplugin_audio_ajaxobject != "undefined" || instance.options.radionomyservice)) {
					...
					if (typeof wonderplugin_audio_ajaxobject != "undefined") {
						...
						args.url = instance.options.radionomyservice;
					$.ajax(args).done(function (data) {
						if (data) {
							var playedlist = '<div class="amazingaudioplayer-playedlist-title">' + instance.options.playedlisttitle + "</div>";
							var count =
							...
							for (var i = 0; i < count; i++) {
								...
								var text = "";
								if (image)
									text += '<div class="amazingaudioplayer-playedlist-item-radionomy-image"><img src="' + image + '" alt="' + amEscapeHtml(title) + '" width="' + instance.options.playedlistimagewidth + '" height="' +
								...
								if (i == 0) {
									if (title)
										instance.elemArray[instance.currentItem].title = title;
									if (artist)
										instance.elemArray[instance.currentItem].artist =
											artist;
									if (image)
		...
		this.each(function () {
			...
			if ($(this).data("skin") && typeof AMAZINGAUDIOPLAYER_SKIN_OPTIONS !== "undefined")
				if ($(this).data("skin")in AMAZINGAUDIOPLAYER_SKIN_OPTIONS)
					this.options = $.extend({}, AMAZINGAUDIOPLAYER_SKIN_OPTIONS[$(this).data("skin")],
							this.options);
			var instance = this;
	...
	$.amascrollbar = function (parent, child, options) {
		...
		scrollbar.click(function (e) {
			...
			var newtop = thumbtop;
			if (top > thumbtop + thumbheight)
				newtop = thumbtop + thumbheight;
			else if (top < thumbtop)
				newtop = thumbtop - thumbheight;
			if (newtop < 0)
				newtop = 0;
			if (newtop > visibleheight - thumbheight)
				newtop = visibleheight - thumbheight;
			scrollthumb.css({
				top: newtop + "px"
			});
			child.css({
		...
		scrollthumb.on("mousedown", function (e) {
			...
			$(document).on("mousemove.amadraggable", function (e) {
				var t = y + e.pageY;
				if (t < scrollbar.offset().top)
					t = scrollbar.offset().top;
				if (t > scrollbar.offset().top + visibleheight - thumbheight)
					t = scrollbar.offset().top + visibleheight - thumbheight;
				dragged.offset({
					top: t
				});
		...
		var scrollChild = function (dis) {
			...
			if (options.enablepagescroll) {
				if (childtop == 0 && dis >= 0)
					return false;
				if (childtop == visibleheight - totalheight && dis <= 0)
					return false
			}
			childtop += dis;
			if (childtop > 0)
				childtop = 0;
			if (childtop < visibleheight - totalheight)
				childtop = visibleheight - totalheight;
			child.css({
			...
			if (thumbtop < 0)
				thumbtop = 0;
			if (thumbtop > visibleheight - thumbheight)
				thumbtop =
		...
		parent.on("DOMMouseScroll mousewheel wheel", function (e) {
			...
			if (e.originalEvent)
				if (e.originalEvent.wheelDeltaY)
					dis = e.originalEvent.wheelDeltaY;
				else if (e.originalEvent.deltaY)
					dis = -e.originalEvent.deltaY;
		...
		if (ua.win && ua.ie)
			...
			if (nestedObj) {
				...
				if (c) {
					...
					for (var i = 0; i < cl; i++)
						if (!(c[i].nodeType ==
								1 && c[i].nodeName == "PARAM") && !(c[i].nodeType == 8))
			...
			window.attachEvent("onunload", function () {
				...
				if (window.AmazingSWFObject) {
					for (var l in window.AmazingSWFObject)
						window.AmazingSWFObject[l] = null;
